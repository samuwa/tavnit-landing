// Generates public/lite/sample-invoice.pdf — the "try it without a file"
// document for /tools/invoice-to-excel. A fictional supplier, six line
// items, mixed units, a discount line and tax, so the extracted table is
// interesting rather than trivial. Run: node scripts/make-sample-invoice.mjs
...
const t = (text, x, y, size = 10, f = font, color = ink) =>
...
tr("No. FAC-2026-004417", 545, 762, 10);
...
const itbms = Math.round(base * 0.07 * 100) / 100;
...
y -= 16;
tr("Descuento pronto pago 3%", cols.price + 30, y, 10);
tr(`- B/. ${discount.toFixed(2)}`, cols.total - 4, y, 10);
y -= 16;
...
y -= 14;
t("Mercancía entregada según nota de entrega ND-2026-1187. Precios en balboas (USD).", 50, y, 9);
y -= 13;
...
writeFileSync("public/lite/sample-invoice.pdf", bytes);
console.log("wrote public/lite/sample-invoice.pdf", bytes.length, "bytes; total", total.toFixed(2));
