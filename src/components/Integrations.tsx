...
function DesktopFeatureTags({ tags }: { tags: string[] }) {
  return (
    <div className="flex flex-wrap gap-1 mt-auto pt-5">
      {tags.map((t) => (
        <span key={t} className="bg-tint/5 border border-tint/10 rounded-md text-fg-4 font-medium px-2.5 py-1 text-[11px]">{t}</span>
      ))}
    </div>
  );
}
...
function DesktopCardLink({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="inline-flex items-center gap-1.5 font-semibold text-accent hover:text-accent-2 transition-colors group/link text-sm mt-5">
      {label}
...
function DesktopEmailSteps({ steps, numbered }: { steps: string[]; numbered?: boolean }) {
  return (
    <div className="space-y-2.5">
      {steps.map((step, i) => (
        <div key={i} className="flex items-start gap-3 text-sm text-fg-3">
          <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 font-bold text-[10px] ${numbered ? "bg-gradient-to-r from-[#3b82f6] to-[#6c42f0] text-white" : "bg-emerald-500/20 text-ok"}`}>
            {numbered ? i + 1 : "✓"}
          </span>
...
function ApiDesktop() {
  return (
    <>
      <div className="bg-well/40 rounded-lg overflow-hidden border border-tint/5">
        <div className="flex justify-between items-center px-4 py-2 border-b border-tint/5">
          <span className="text-[10px] font-bold text-accent uppercase tracking-widest">cURL</span>
          <span className="text-[10px] text-fg-5">Quick Example</span>
        </div>
        <pre className="p-4 overflow-x-auto">
          <code className="font-mono text-[11px] leading-[1.7] text-ok block whitespace-pre">{`curl -X POST /api/runs/process \\
  ...
  -F "flow_id=YOUR_FLOW_ID"`}</code>
      ...
      <div className="mt-4">
        <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-2.5">More Endpoints</h4>
        <div className="space-y-1.5">
          {apiEndpoints.map((e) => (
            <div key={e.path} className="flex items-center gap-2.5 text-[12px]">
              <span className="px-1.5 py-0.5 rounded bg-[#3b82f6]/10 text-accent font-mono font-bold text-[9px] flex-shrink-0">POST</span>
              <code className="font-mono text-fg-3 flex-shrink-0">{e.path}</code>
              <span className="text-fg-5 truncate">— {e.desc}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <>
      <div className="space-y-5">
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-3">Email Trigger</h4>
        ...
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-3">Email Output</h4>
          <DesktopEmailSteps steps={["Configure recipient email in flow settings", "Receive JSON results automatically"]} />
        </div>
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-2.5">Accepted Attachments</h4>
          <div className="flex items-center flex-wrap gap-1.5 mb-2">
            {[".pdf", ".png", ".jpg", ".tiff", ".webp", "+ more"].map((ext) => (
              <span key={ext} className="px-2 py-0.5 rounded bg-well/40 border border-tint/10 text-fg-3 font-mono text-[10px]">{ext}</span>
            ))}
          </div>
          <p className="text-[12px] text-fg-5 leading-snug">
...
function WebhookDesktop() {
  return (
    <>
      <div className="p-4 bg-tint/3 rounded-xl border border-tint/5">
          ...
          {[
            ...
            <div key={i} className="flex items-start flex-1">
              <div className="flex flex-col items-center gap-2 flex-1">
                <div className="w-10 h-10 rounded-full bg-tint/5 border border-tint/10 flex items-center justify-center text-accent shadow-sm">
                  <item.icon size={18} />
                </div>
                <span className="text-[11px] text-fg-4 font-medium text-center leading-tight max-w-[90px]">{item.label}</span>
              </div>
              {i < 2 && (
      ...
      <div className="mt-4 bg-well/40 rounded-lg overflow-hidden border border-tint/5">
        <div className="flex justify-between items-center px-4 py-2 border-b border-tint/5">
          <span className="text-[10px] font-bold text-accent uppercase tracking-widest">Payload</span>
          <span className="text-[10px] text-fg-5">Sent to your URL</span>
        </div>
        <pre className="p-3.5 overflow-x-auto">
          <code className="font-mono text-[11px] leading-[1.6] text-ok block whitespace-pre">{`{
  "vendor": "Acme Corp",
  ...
  "flow_id": "9b2e51c0-…"
      ...
      <p className="text-[12px] text-fg-5 leading-snug mt-3">
...
function McpDesktop() {
  return (
    <>
      <div className="space-y-5">
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-3">Connect in Minutes</h4>
        ...
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-3">What Your Assistant Can Do</h4>
          <DesktopEmailSteps steps={["Process documents through your flows", "Read and search your Buckets"]} />
        </div>
        <div>
          <h4 className="text-xs font-bold text-fg-5 uppercase tracking-wider mb-2.5">Try Asking</h4>
          <div className="space-y-1.5">
            {[
              "“Run this invoice through my Supplier Invoices flow”",
              "“What did we pay Acme Corp last quarter?”",
            ].map((prompt) => (
              <div key={prompt} className="px-3 py-1.5 rounded-lg bg-well/40 border border-tint/10 text-[12px] text-fg-3 italic">
                {prompt}
              </div>
            ))}
          </div>
      ...
      {steps.map((step, i) => (
        <div key={i} className="flex items-start gap-2 text-xs text-fg-3">
          <span className={`w-[18px] h-[18px] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 font-bold text-[9px] ${
            numbered ? "bg-gradient-to-r from-[#3b82f6] to-[#6c42f0] text-white" : "bg-emerald-500/20 text-ok"
          }`}>
            {numbered ? i + 1 : "✓"}
          </span>
          <span className="leading-snug">{step}</span>
    ...
    <section className="py-16 md:py-24" id="integrations" aria-labelledby="integrations-heading">
      ...
      <div className="hidden md:block max-w-[1100px] mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 md:mb-10">
          <h2 id="integrations-heading" className="text-2xl md:text-4xl font-bold text-fg mb-1 md:mb-3">Multiple Ways to Integrate</h2>
          <p className="text-sm md:text-lg text-fg-4 max-w-[540px] mx-auto">
            Choose the integration method that fits your workflow
          </p>
        </div>
                ...
                <button
                  ...
                  className={`glass-card rounded-xl px-4 py-3.5 flex items-center gap-3.5 text-left transition-all duration-300 cursor-pointer flex-1 ${
                    active
                      ...
                      : "hover:border-tint/20 hover:bg-tint/[0.03]"
                  }`}
                >
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                      active
                        ...
                        : "bg-[#3b82f6]/10 text-accent"
                    }`}
                  >
                    <m.icon size={19} />
                  </div>
                  <div className="min-w-0">
                    <div className={`text-sm font-bold transition-colors ${active ? "text-fg" : "text-fg-3"}`}>
                      {m.title}
                    </div>
                    <div className="text-xs text-fg-5 truncate">{m.subtitle}</div>
                  </div>
                  <ArrowRight
                    size={15}
                    className={`ml-auto flex-shrink-0 transition-all duration-300 ${
                      active ? "text-accent opacity-100 translate-x-0" : "opacity-0 -translate-x-1"
                    }`}
                  />
                </button>
              );
