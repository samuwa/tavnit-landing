...
export async function quotaSnapshot(sessionId: string, ipHash: string): Promise<QuotaSnapshot> {
  const [session, ip, global] = await Promise.all([
    countSince(`&session_id=eq.${encodeURIComponent(sessionId)}`),
    countSince(`&ip_hash=eq.${encodeURIComponent(ipHash)}`),
    countSince(""),
  ]);
  ...
  return {
    ...
    ip,
    global,
    limitPerDay: LITE_LIMITS.runsPerDay,
...
export async function getRunForSession(
  ...
  if (!h || !b) return null;
  const res = await fetch(
    `${b}/rest/v1/lite_runs?run_id=eq.${encodeURIComponent(runId)}&session_id=eq.${encodeURIComponent(sessionId)}&select=*&limit=1`,
